...
if(isset($_POST['tid'])){
	...
	$query = $conn->prepare("UPDATE `teacherinfo` SET `name`= ? ,`username`= ?, `img` = ? , `sub1` = ?,`sub2` = ?,`sub3` = ?,`sub4` = ?,`sub5` = ? WHERE `tid`= ?");
	$query->bind_param("sssiiiiii",$tname,$username,$img,$sub1,$sub2,$sub3,$sub4,$sub5,$tid);
	$query->execute();
	}
	else{
	$encpass = password_hash($pass, PASSWORD_BCRYPT);
	$query = $conn->prepare("UPDATE `teacherinfo` SET `name`= ?, `username`= ?,`password`= ?, `img` = ? , `sub1` = ?,`sub2` = ?,`sub3` = ?,`sub4` = ?,`sub5` = ? WHERE `tid`= ?");
	$query->bind_param("ssssiiiiii",$tname,$username,$encpass,$img,$sub1,$sub2,$sub3,$sub4,$sub5,$tid);
	$query->execute();
	}
	...
	$adnul4->execute();
	$adnul5->execute();
	header('Location:index.php?p=teacher');
